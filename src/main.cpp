...
int segunda ( char *letras, char *numeros )
{
    ...
    while ( (img = cvLoadImage(strcat(num, ex), 0)) != 0 )
    {
        if ( img->width < img->height )
        {
            ...
            if ( cl >= 48 && cl<= 57 ) retorno[i-contL - 3] = cl;
        }
        sprintf(num, "%s%d", ger, i);
        i++;
    }
...
int principal()
{
    ...
    while ( src == NULL )
    {
        ...
        if ( src == NULL ) cout << "Imagem nao encontrada!\n";
    }
...
{
    char retorno ='*';
    if ( car[0] == car[1] && car[0] == car[2] && car[0] == car[3] ) return car[0];
    else if ( (car[0] == car[1] && car[0] == car[2]) || (car[0] == car[2] && car[0] == car[3]) )return car[0];
    else if ( (car[1] == car[2] && car[1] == car[3]) || (car[1] == car[0] && car[1] == car[3]) ) return car[1];
    else if (car[0] == car[1] && car[2] != car[3]) return car[0];
    else if (car[0] == car[2] && car[1] != car[3]) return car[0];
    else if (car[0] == car[3] && car[1] != car[2]) return car[0];
    else if (car[1] == car[2] && car[0] != car[3]) return car[1];
    else if (car[1] == car[3] && car[0] != car[2]) return car[1];
    else if (car[2] == car[3] && car[0] != car[1]) return car[2];
    else if (car[0] == car [1] && car[2] == car[3] )
    {
        if ( car[0] == charMedia ) return car[0];
        else return car[2];
    }
    else if ( car[0] == car [1] && car[2] == car[3] )
    {
...
void pegaDireita( IplImage *imagem, int *ladoDireito, double it )
{
    ...
    for ( double  i = 0.0; i < imagem->height; i += it )
    {
        ...
        while ( ((uchar*)( imagem->imageData + imagem->widthStep * (int) i))[ j ] > 10 && j > imagem->width/2 ) j--;
        ladoDireito[ k++ ] = j;
    }
}
...
void pegaTopo( IplImage *imagem, int *vetorTopo, double it )
{
    ...
    {
        int j = 0;
        while ( ((uchar*)( imagem->imageData + imagem->widthStep * j ))[ (int )i ] >10 && j < imagem -> height /2 ) j++;
        vetorTopo[ k++ ] =  j;
    }
}
...
double verificaLados( int a[], int b[], int tamanho )
{
    ...
    int i, cont = 0;
    for ( i = 0 ; i < tamanho ; i++ )
        if ( ( a[i]- da ) <= b[i] - db && b[i] - db <= ( a[i] - da ) ) cont++;
...
double verificaBases( int a[], int b[], int tamanho )
{
    ...
    int i ,cont = 0;
    for ( i = 0; i < tamanho; i++ )
        if ( ( a[i] - da - 2 ) <= b[i]-db && b[i] - db <= ( a[i] - da + 2 ) ) cont++;
    return (double) cont / (double) i;
}
